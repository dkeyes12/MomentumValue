...
# 1. Add sidebar selector for RSI source
...
def calculate_rsi(data_series, window=14):
    """
    Calculates the Relative Strength Index (RSI) for a given data series (e.g., Open, Close, High).
    """
...
def get_stock_data(ticker, period, source_column):
    try:
        ...
        # Calculate VWAP if needed later, otherwise volume is already there.
        # Note: yfinance history doesn't directly provide VWAP without manual calculation per timeframe.
        # We stick to OHLCV columns available by default.

        # Calculate Indicators (SMA calculations use Close price by default)
        df['SMA_50'] = df['Close'].rolling(window=50).mean()
        df['SMA_200'] = df['Close'].rolling(window=200).mean()
        
        # 3. Use the user-selected source_column to calculate RSI
        if source_column in df.columns:
            df['RSI'] = calculate_rsi(df[source_column])
        else:
            # Fallback if the requested column isn't available (e.g. asking for VWAP without calculating it)
...
with st.spinner('Fetching market data...'):
    # Pass the selected RSI source to the get_stock_data function
...
if df is not None and not df.empty:
    ...
    if pe_ratio is None:
        pe_ratio = info.get('forwardPE') # Fallback
    
    # Determine Signal
    # Note: The signal uses the RSI calculated from the user-selected source
    ...
    with col1:
        st.metric("Current Price", f"${current_price:.2f}", f"{current_price - df['Close'].iloc[-2]:.2f}")
    with col2:
        st.metric(f"RSI (14) on {rsi_source}", f"{last_rsi:.2f}") # Updated metric label
    with col3:
    ...
    with col4:
        st.markdown(f"""
            <div style="text-align: center; padding: 10px; background-color: #f0f2f6; border-radius: 10px;">
                ...
                <h2 style="margin:0; color: {('green' if signal=='BUY' else 'red' if 'SELL' in signal else 'orange')};">{signal}</h2>
            </div>
    ...
    if pe_ratio is not None and pe_ratio > 0:
        # ... (rest of the quadrant plot code remains the same as it uses the `last_rsi` variable) ...
        # Define Thresholds
        PE_THRESHOLD = 25  # P/E crossing point (Market Avg approx)
        RSI_THRESHOLD = 50 # RSI crossing point
        ...
        # Add Quadrant Backgrounds (The 2x2 Grid)
        ...
        fig_quad.add_shape(type="rect", x0=PE_THRESHOLD, y0=RSI_THRESHOLD, x1=pe_ratio*2 if pe_ratio > 50 else 100, y1=100,
        ...
        fig_quad.add_shape(type="rect", x0=PE_THRESHOLD, y0=0, x1=pe_ratio*2 if pe_ratio > 50 else 100, y1=RSI_THRESHOLD,
        ...
        fig_quad.update_yaxes(title_text=f"RSI ({rsi_source} data) (Momentum)", range=[0, 100]) # Updated Y-axis label
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], 
                             line=dict(color='purple', width=2), name=f'RSI ({rsi_source})'), row=2, col=1) # Updated legend name
