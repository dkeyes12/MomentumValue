...
def get_stock_data(ticker, period, source_column):
    try:
        # 1. Fetch Data using YFinance (Standard & Reliable)
        ...
        # 2. Get Fundamentals (P/E Ratio)
        info = stock.info
        # Try trailing, then forward P/E
        pe_ratio = info.get('trailingPE') or info.get('forwardPE')

        # 3. Technical Analysis Calculations (Manual Pandas)
        # Ensure we use the correct column (Title Case for yfinance: 'Close', 'Open')
        target_col = source_column.title()
...
st.title(f"📊 {ticker_input} Dashboard (Lite)")
...
if df is not None and not df.empty:
    ...
    # Create Subplots: Price (70%), RSI (30%)
    fig = make_subplots(
        rows=2, cols=1, 
        shared_xaxes=True, 
